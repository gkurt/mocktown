...
import type { EkbEntry } from "../contract/schemas.ts";
import { captureEnv, JAVA_GUIDANCE } from "../capture/launch.ts";
...
export function generateEnv(inputs: EnvInputs): EnvArtifacts {
  ...
  for (const service of [...new Set(inputs.services)].sort()) {
    ...
    if (entries.length === 0) {
      report.push({
        service, rung: null, covered: false,
        how: baseUrl
    ...
    report.push({
      service, rung: best.rung, covered: false,
      how: best.rung === 1
