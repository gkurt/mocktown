...
import { inputShape, walkContract, type ProcedureInfo } from "../contract/walk.ts";
import { clientFor, ensureDaemon } from "./daemon-client.ts";
import { renderResult } from "./render.ts";
import { resolveProject, ensureRegistered, loadGlobalConfig, saveGlobalConfig } from "../config/project.ts";
import { ProjectFile } from "../config/schema.ts";
import { launch } from "../capture/launch.ts";
...
const collapsedProcedures = new Set(
  procedures
    ...
    .filter((p) => {
      ...
      return Object.entries(shape).every(([name, field]) =>
        name === "project" || name === "source" || coreType(field).optional);
    })
    .map((p) => p.path.join(".")),
);
...
function commandFor(procedure: ProcedureInfo): Command {
  ...
  for (const segment of segments.slice(0, -1)) {
    parent = parent.commands.find((c) => c.name() === segment)
      ?? parent.command(segment).description(GROUP_DESCRIPTIONS[segment] ?? "");
  }
...
daemonCommand
  ...
  .action(() => {
    const state = readDaemonState();
    if (!state) { console.log("not running"); return; }
...
program
  ...
  .action(async () => {
    const { runMcpServer } = await import("../mcp/index.ts");
    await runMcpServer();
  });
