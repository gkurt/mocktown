...
import { normalizeUrl, stripVolatile } from "./normalize.ts";
import { hash, id } from "../util/id.ts";
...
export class Recorder {
  ...
  record(exchange: CapturedExchange, source: "front-door" | "har" = "front-door"): RecordedRow | null {
    ...
    this.db.insert(schema.recordings).values({
      id: rowId,
...
export function startSession(db: Db, mode: "record" | "serve" | "import" | "verify", opts: { seed?: string; label?: string } = {}): string {
  ...
  db.insert(schema.sessions).values({ id: sessionId, mode, seed, label: opts.label ?? null }).run();
  db.insert(schema.journal).values({ id: id("jrn"), sessionId, kind: "session-start", payload: { mode, seed } }).run();
  return sessionId;
}
