...
import { globalConfigDir, daemonStateFile } from "../config/paths.ts";
import { findFreePort } from "../util/ports.ts";

const handler = new OpenAPIHandler(router);

export const openapi = await new OpenAPIGenerator({ schemaConverters: [new ZodToJsonSchemaConverter()] })
  .generate(contract, {
    info: {
...
function writeDaemonState(port: number, token: string): void {
  ...
  writeFileSync(daemonStateFile(), JSON.stringify({ port, token, pid: process.pid, startedAt: new Date().toISOString() }, null, 2), { mode: 0o600 });
}
    ...
    async fetch(request) {
      ...
      if (url.pathname.startsWith("/api/")) {
        if (request.headers.get("authorization") !== `Bearer ${token}`) {
          return Response.json({ error: "unauthorized", message: "Set Authorization: Bearer <token> from ~/.config/mocktown/daemon.json" }, { status: 401 });
        }
      }
