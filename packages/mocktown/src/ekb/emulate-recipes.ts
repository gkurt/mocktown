...
export const EMULATE_RECIPES: Record<string, EndpointRecipe[]> = {
  ...
  okta: [{ rung: 3, note: `Point the app's Okta issuer/org URL at the emulator's base URL. ${CONSENT_NOTE}` }],

  clerk: [{ rung: 3, note: `Point the Clerk client's API base at the emulator's base URL. ${CONSENT_NOTE}` }],

  google: [
    {
      rung: 3,
      note: `Google SDKs resolve endpoints per API; override the discovery/base URL for the specific API in use. ${CONSENT_NOTE}`,
    },
  ],

  vercel: [{ rung: 3, note: "Set the Vercel SDK's server URL option to the emulator's base URL; there is no standard env var." }],

  linear: [{ rung: 3, note: "Point the Linear client's GraphQL endpoint at the emulator's base URL." }],
