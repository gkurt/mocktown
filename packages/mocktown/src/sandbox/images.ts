  ...
  return [
    ...
    ...(options.browser
      ? [
          ...
          '# Playwright, Puppeteer and agent-browser all take an executable path from the',
          '# environment, so a browser test written for a laptop runs unchanged inside the',
          '# boundary. Each reads its own variable, and a driver that finds none of them',
          '# downloads a browser — which on a sealed network is a hang, not an error.',
