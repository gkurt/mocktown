...
import { z } from "zod";
import { GlobalConfig, LocalConfig, ProjectFile } from "./schema.ts";
import { globalConfigDir, globalConfigFile, projectDataDir, workspacePaths } from "./paths.ts";
...
export function resolveProject(opts: { project?: string; cwd?: string } = {}): ResolvedProject {
  ...
  if (opts.project) { name = opts.project; source = "flag"; }
  else if (process.env.MOCKTOWN_PROJECT) { name = process.env.MOCKTOWN_PROJECT; source = "env"; }
  else if (file) { name = file.project; source = "file"; }
  else { name = loadGlobalConfig().defaultProject; source = "default"; }
