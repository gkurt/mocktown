 ...
 */
import type { Recording } from "../contract/schemas.ts";
import type { Scrubber } from "../scrub/scrubber.ts";
...
export async function verifyRecordings(
  ...
  for (const recording of recordings) {
    ...
    try {
      ...
      failures.push({
        recordingId: recording.id, method: recording.method, path: recording.path,
        reason: `request failed: ${error instanceof Error ? error.message : String(error)}`,
        expectedStatus: recording.statusCode, actualStatus: null, diff: [],
      });
      continue;
    }
    ...
    if (Math.floor(response.status / 100) !== Math.floor(recording.statusCode / 100)) {
      failures.push({
        ...
        expectedStatus: recording.statusCode, actualStatus: response.status,
    ...
    if (recordedBody && isJson) {
      ...
      try { expected = JSON.parse(recordedBody); } catch { expected = null; }
      try { actual = JSON.parse(text); } catch { actual = null; }
