...
import { DEFAULT_RULES } from "../scrub/rules.ts";
import { Prng, streamKey } from "./prng.ts";
import { SqliteStateStore } from "./state.ts";
import { diagnose, matchRoute } from "./match.ts";
import type { MockCtx, MockModule, MockRequest, MockResponse } from "./types.ts";
...
export class MockHost {
  ...
  constructor(private readonly deps: MockHostDeps) {}
  ...
  private async handle(request: Request): Promise<Response> {
    ...
    if (!module) {
      ...
      this.deps.onUnmatched({
        service, method: request.method, path: url.pathname,
  ...
  private contextFor(service: string, profile: string, endpoint: string, request: MockRequest): MockCtx {
    ...
    const prng = new Prng(streamKey({
      sessionSeed: this.deps.sessionSeed, service, endpoint, profile, requestIdentity,
    }));
    ...
    return {
      ...
      signIn: (credentials) => {
        ...
        const matched = roster.find((p) =>
          Object.entries(p.credentials).length > 0 &&
          Object.entries(p.credentials).every(([key, value]) => credentials[key] === value));
        if (!matched) return null;
        const token = `mtk_${id("s").slice(2)}`;
        db.insert(schema.profileSessions)
          .values({ token, profile: matched.name, sessionId: this.deps.sessionId })
          .run();
