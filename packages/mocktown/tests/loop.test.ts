...
beforeAll(async () => {
  ...
  writeFileSync(join(workspace, "mocktown.json"), JSON.stringify({
    project: "loop-test",
...
describe("phase 1 — record a real app's traffic, browse the scrubbed corpus", () => {
  test("records exchanges through the front door", async () => {
    ...
    await fetch(`${base}/v1/invoices/${created.id}`, { proxy: proxyUrl, headers: { authorization: "Bearer sk_test_51QxSpIkeRealLookingSecretKeyABCDEF" } });
    await fetch(`${base}/v1/invoices`, { proxy: proxyUrl, headers: { authorization: "Bearer sk_test_51QxSpIkeRealLookingSecretKeyABCDEF" } });
  ...
  test("the corpus is normalized at write time", () => {
    const templates = new Set(runtime.db.select().from(schema.recordings).all().map((r) => r.pathTemplate));
...
describe("phase 2 — the loop closes on a real project", () => {
  ...
  test("the mock is stateful, not a replay of recorded bodies", async () => {
    const base = `http://${SERVICE}:${UPSTREAM_PORT}`;
    const created = await fetch(`${base}/v1/invoices`, {
      method: "POST", proxy: proxyUrl,
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ amount: 999 }),
    }).then((r) => r.json() as Promise<{ id: string; amount: number }>);

    const fetched = await fetch(`${base}/v1/invoices/${created.id}`, { proxy: proxyUrl })
      .then((r) => r.json() as Promise<{ id: string; amount: number }>);
