...
describe("mocktown env", () => {
  const inputs = {
    ...
    baseUrls: new Map([["api.github.com", "http://127.0.0.1:4601"], ["api.twilio.com", "http://127.0.0.1:4602"]]),
    ekb: [
      { id: "1", service: "api.github.com", rung: 1, envVar: "GITHUB_API_URL", language: null, snippet: null, note: null, source: "emulate-skill" },
      { id: "2", service: "api.twilio.com", rung: 3, envVar: null, language: null, snippet: null, note: "no endpoint option", source: "emulate-skill" },
    ],
...
describe("issues as files", () => {
  ...
  test("a recurrence refreshes the diagnosis and the fix together", () => {
    ...
    const issueId = runtime.issues.file({
      type: "unmatched-request", service: "c.test", method: "GET", pathTemplate: "/z",
      diagnosis: { reason: "first reason" }, suggestedResolution: "first fix",
    });
    runtime.issues.file({
      type: "unmatched-request", service: "c.test", method: "GET", pathTemplate: "/z",
      diagnosis: { reason: "second reason" }, suggestedResolution: "second fix",
    });
