...
let testNo = 0;
const skip = () => { testNo++; return (ONLY !== null && ONLY !== testNo) || SKIP.has(testNo); };
...
if (!skip()) try {
  const { stdout } = await execFileP("curl", ["-sS", "--max-time", "15", "--proxy", PROXY, "--cacert", "out/ca.pem",
    "-o", "/dev/null", "-w", "%{http_code} %{http_version}", "https://example.com"]);
  const [code, ver] = stdout.trim().split(" ");
  check("HTTPS MITM (h1.1, real upstream)", code === "200", `status=${code} clientHttpVersion=${ver}`);
} catch (e: any) { check("HTTPS MITM (h1.1, real upstream)", false, e.message?.slice(0, 160)); }

// ── 2. HTTP/2 client through the MITM ─────────────────────────────────────────
if (!skip()) try {
  const { stdout } = await execFileP("curl", ["-sS", "--http2", "--max-time", "15", "--proxy", PROXY, "--cacert", "out/ca.pem",
    "-o", "/dev/null", "-w", "%{http_code} %{http_version}", "https://cloudflare.com"]);
  const [code, ver] = stdout.trim().split(" ");
  const seenH2 = observed.some((o) => o.url.includes("cloudflare.com") && o.httpVersion === "2.0");
  check("HTTP/2 client through MITM", ver === "2" && seenH2,
    `clientHttpVersion=${ver} status=${code} proxyObservedVersion=${observed.find((o) => o.url.includes("cloudflare"))?.httpVersion}`);
} catch (e: any) { check("HTTP/2 client through MITM", false, e.message?.slice(0, 160)); }
...
if (!skip()) try {
  ...
  const body = await new Promise<string>((resolve, reject) => {
    const req = https.request({ host: "mocked.example", path: "/api/orders", agent, ca: [ca.cert] } as any,
      (res) => { let b = ""; res.on("data", (d) => (b += d)); res.on("end", () => resolve(`${res.statusCode} ${b}`)); });
    req.on("error", reject);
    req.setTimeout(10000, () => req.destroy(new Error("timeout")));
    req.end();
  });
  check("Mock served for non-existent host", body.includes(`"mocked":true`), body);
} catch (e: any) { check("Mock served for non-existent host", false, e.message?.slice(0, 160)); }
...
if (!skip()) try {
  ...
  const body = await new Promise<string>((resolve, reject) => {
    const req = https.request({ host: "localhost", port: 9443, path: "/ping", agent, rejectUnauthorized: false } as any,
      (res) => { let b = ""; res.on("data", (d) => (b += d)); res.on("end", () => resolve(`${res.statusCode} ${b}`)); });
    req.on("error", reject);
    req.setTimeout(10000, () => req.destroy(new Error("timeout")));
    req.end();
...
} catch (e: any) { check("Passthrough to local TLS upstream", false, e.message?.slice(0, 160)); }
...
} catch (e: any) { check("WebSocket over TLS through MITM", false, (e.message ?? String(e)).slice(0, 160)); }

// ── 6. Plain HTTP through the same port (the front door is one port) ──────────
if (!skip()) try {
  const body = await new Promise<string>((resolve, reject) => {
    const req = http.request({ host: "127.0.0.1", port: 8123, path: "http://example.com/", method: "GET",
      headers: { host: "example.com" } },
      (res) => { res.resume(); res.on("end", () => resolve(String(res.statusCode))); });
    req.on("error", reject);
    req.setTimeout(10000, () => req.destroy(new Error("timeout")));
    req.end();
  });
  check("Plain HTTP proxying on the same port", body === "200", `status=${body}`);
} catch (e: any) { check("Plain HTTP proxying on the same port", false, e.message?.slice(0, 160)); }
