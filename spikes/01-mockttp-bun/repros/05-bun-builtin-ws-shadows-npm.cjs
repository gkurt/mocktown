...
for (const [label, base] of [
  ['from this file', __filename],
  ["from mockttp's own file", mockttpFile],
]) {
  const req = createRequire(base);
  let resolved;
  try {
    resolved = req.resolve('ws');
  } catch (e) {
  ...
  console.log(
    `  ${real ? 'PASS' : 'FAIL'}  ${label.padEnd(24)} resolved=${resolved.length > 55 ? `…${resolved.slice(-42)}` : resolved}  realNpmWs=${real}`,
  );
}
