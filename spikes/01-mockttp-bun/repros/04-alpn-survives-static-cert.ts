...
tlsServer.on('secureConnection', (s) => {
  negotiatedServerSide = String(s.alpnProtocol);
  s.end();
});
...
const chosen = await new Promise<string>((resolve) => {
  ...
  c.on('error', (e) => {
    clearTimeout(t);
    resolve(`ERROR ${e.message.slice(0, 60)}`);
  });
});
await new Promise((r) => setTimeout(r, 100));
console.log(
  `  ${chosen === 'h2' ? 'PASS' : 'FAIL'}  static cert + static ALPN via handoff: client=${chosen} server=${negotiatedServerSide}`,
);
process.exit(0);
