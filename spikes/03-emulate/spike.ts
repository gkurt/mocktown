...
try {
  ...
  check(
    'Provider starts emulate + reports base URLs',
...
const repo: any = await (
  await fetch(`${githubUrl}/repos/octocat/hello-world`, { headers: { authorization: 'Bearer test_token_admin' } })
).json();
const seededCustomer = customers.data?.some((c: any) => c.email === 'ada@example.com');
check(
  'Seed file applied to both services',
...
try {
  ...
  check(
    'Real Stripe SDK against the emulator',
  ...
  check('Real Stripe SDK against the emulator', false, (e.message ?? String(e)).slice(0, 160));
}
...
try {
  ...
  check('Real Octokit against the emulator', false, (e.message ?? String(e)).slice(0, 160));
}
...
try {
  ...
  const tokenRes = await fetch(`${githubUrl}/login/oauth/access_token`, {
    ...
    body: JSON.stringify({
      client_id: 'Iv1.mocktown_spike',
      client_secret: 'mocktown_spike_secret',
      code,
      redirect_uri: 'http://127.0.0.1:9999/callback',
    }),
  });
  ...
  check(
    'OAuth end-to-end, token authenticates an SDK call',
  ...
  check('OAuth end-to-end, token authenticates an SDK call', false, (e.message ?? String(e)).slice(0, 180));
}
...
if (lan) {
  try {
    ...
    reachableOffLoopback = false;
  }
}
check(
  'emulate ports are loopback-only',
  !reachableOffLoopback,
...
check(
  'stop() terminates the child and frees the port',
  !provider.isRunning && !stillServing,
...
try {
  ...
  const customers: any = await (
    await fetch(`${restarted.get('stripe')}/v1/customers`, { headers: { authorization: 'Bearer sk_test_x' } })
  ).json();
  ...
  check(
    'Restart is reproducible from the seed',
    emails.includes('ada@example.com') && !emails.includes('grace@example.com'),
    `customers=[${emails.join(', ')}]`,
  );
  await second.stop();
} catch (e: any) {
  check('Restart is reproducible from the seed', false, (e.message ?? String(e)).slice(0, 160));
}
