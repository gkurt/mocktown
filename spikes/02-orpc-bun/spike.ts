 ...
 *   bun run spike.ts
 */
import { startServer, openapi } from "./server.ts";
...
// ── 1. OpenAPIHandler on Bun.serve, GET with query params ─────────────────────
{
  ...
  check("OpenAPIHandler on Bun.serve (GET)", res.status === 200 && body.services?.length > 0,
...
// ── 2. PUT with a path parameter and a JSON body ──────────────────────────────
{
  ...
  const body: any = await res.json();
  check("PUT with path param + body", res.status === 200 && body.service?.provider === "emulator:slack",
...
// ── 4. Static GUI files from the same Bun.serve, no framework ─────────────────
{
  ...
  check("Static files on the same server", res.status === 200 && html.includes("Mocktown GUI shell"),
...
// ── 6. OpenAPI document generated from the contract ───────────────────────────
{
  ...
  check("OpenAPI generated from contract",
...
{
  const client: any = createORPCClient(new OpenAPILink(contract, {
    url: BASE, headers: () => authed,
  }));
  const out = await client.services.list({ project: "acme-api" });
  check("Typed oRPC client round-trip", out.project === "acme-api" && Array.isArray(out.services),
...
// ── 8. The contract walk finds every procedure ────────────────────────────────
{
  ...
  check("Contract walk enumerates procedures", procs.length === 2 && procs.every((p) => p.inputSchema && p.route),
...
// ── 9. CLI generated from the contract (spawned as a real process) ────────────
{
  ...
  let ok = false, detail = stderr.trim().slice(0, 90);
  try { const parsed = JSON.parse(stdout.trim()); ok = parsed.project === "acme-api"; detail = `${parsed.services.length} services`; } catch {}
...
// ── 10. MCP tools generated from the same walk ────────────────────────────────
{
  ...
  check("MCP server generated from the contract",
