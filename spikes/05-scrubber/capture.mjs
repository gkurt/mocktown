...
await new Promise((resolve, reject) => {
  ...
  const onData = (b) => {
    ...
    if (urls.has('stripe') && urls.has('github') && urls.has('google')) {
      clearTimeout(timer);
      resolve();
    }
  };
...
proxy.on('response', async (res) => {
  ...
  exchanges.push({
    ...
    responseBody: (await res.body.getText()) ?? '',
  });
});
...
const step = async (name, fn) => {
  try {
    ...
    console.log(`  capture step failed (recorded anyway): ${name}: ${(e.message ?? e).toString().slice(0, 110)}`);
  }
};
...
await step('app request carrying PII', async () => {
  await request(
    {
      host: 'api.stripe.com',
      path: '/v1/customers',
  ...
  const consent = await request(
    {
      ...
      path: '/o/oauth2/v2/auth/callback',
      method: 'POST',
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
    },
    new URLSearchParams({
