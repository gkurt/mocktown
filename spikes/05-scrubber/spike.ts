...
import { DEFAULT_RULES, type Exchange, looksHighEntropy, Scrubber } from './scrubber.ts';
...
// 10-security.md: "Scrub at capture time, before disk. The raw exchange never persists."
{
  ...
  check(
    'No known secret survives scrubbing',
    leaked.length === 0,
...
// aren't in secrets.json. They must be redacted anyway.
{
  ...
  check(
    'Server-issued tokens redacted too',
    issued.length > 0 && survivors.length === 0,
...
// placeholder, so they must all carry the same one.
{
  ...
  check(
    'Same secret → same placeholder in a session',
...
// ── 4. Different secrets get different placeholders ───────────────────────────
{
  ...
  check('Distinct secrets are distinguishable', distinctKinds.length >= 4, `kinds=[${distinctKinds.join(', ')}]`);
}
...
// scrubber that matches "token" too eagerly would destroy it.
{
  ...
  check(
    'Non-secret data survives intact',
    decoyIntact && emailsIntact && idsIntact,
...
// the scheme is protocol shape, not a secret.
{
  ...
  check('Auth scheme kept, credential replaced', auths.length >= 2 && schemesKept, auths.length ? auths[0].slice(0, 52) : 'none');
}
...
// 10-security.md: mocks "re-inject fake credentials on replay".
{
  ...
  check(
    'Replay re-injects well-formed fakes',
...
// ── 10. `scrub audit` finds nothing in a correctly scrubbed corpus ────────────
{
  ...
  check(
    'Audit finds no residue in the scrubbed corpus',
    findings.length === 0,
    findings.length
      ? `${findings.length} residual: ${findings
          .slice(0, 2)
...
// the PAT is still caught, by shape alone.
{
  ...
  check(
    'Entropy backstop covers a removed vendor rule',
    !covered.includes(SECRETS.githubPat),
...
// and once it is gone, `mocktown scrub audit` must report the leak.
{
  ...
  check(
    'Audit reports what a stripped rule set leaves behind',
...
// A backstop that fires on ordinary identifiers would quietly wreck the corpus.
{
  ...
  const shouldFire = ['8f14e45fceea167a5a36dedd4bea2543deadbeefcafef00d', 'xapi_live_9f8e7d6c5b4a3928170612345678'];
  const falsePositives = shouldNotFire.filter(looksHighEntropy);
  const missed = shouldFire.filter((v) => !looksHighEntropy(v));
  check(
    'Entropy backstop: no false positives',
...
// ── 14. Redaction metadata is recorded without values ─────────────────────────
{
  ...
  check('Summary records kinds and counts, never values', hasCounts && noValues, summary.map((s) => `${s.kind}×${s.count}`).join(' '));
}
