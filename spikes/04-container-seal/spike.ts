...
async function docker(args: string[], opts: { allowFail?: boolean } = {}) {
  ...
  if (code !== 0 && !opts.allowFail) throw new Error(`docker ${args.slice(0, 3).join(" ")} failed (${code}): ${stderr.trim().slice(0, 300)}`);
...
await docker(["run", "-d", "--name", FRONT_DOOR, "--network", SEALED_NET,
  "--cap-add", "NET_ADMIN",
  "-v", `${process.cwd()}/out:/ca:ro`, "-v", `${process.cwd()}/out:/out`,
  "mocktown-spike-frontdoor"]);
await docker(["network", "connect", EGRESS_NET, FRONT_DOOR]);

const frontDoorIp = (await docker(["inspect", "-f",
  `{{(index .NetworkSettings.Networks "${SEALED_NET}").IPAddress}}`, FRONT_DOOR])).stdout;
...
await docker(["run", "-d", "--name", APP, "--network", SEALED_NET, "--dns", frontDoorIp,
  "mocktown-spike-app", "sleep", "600"]);
...
// broken". Same image, same script, ordinary bridge network.
{
  ...
  await docker(["run", "-d", "--name", "mocktown-spike-control", "--network", EGRESS_NET,
    "mocktown-spike-app", "sleep", "120"]);
  ...
  try { attempts = JSON.parse(r.stdout); } catch {}
  ...
  check("Negative control: the same attempts escape when unsealed", ipv4Escaped.length > 0,
...
// ── 1. DNS override: every hostname resolves to the front door ────────────────
{
  ...
  check("DNS override sends every hostname to the front door",
...
// ── 2. Baked CA: unmodified TLS client, no -k, no proxy config ────────────────
{
  ...
  check("Unmodified code hits mocks with TLS trusted",
...
// ── 3. Unknown host: denied, and the attempt becomes evidence ─────────────────
{
  ...
  let filed = false;
  try { filed = JSON.parse(log.stdout).some((h: any) => String(h.url).includes("exfiltrate.evil.example")); } catch {}
  check("Unknown host denied and filed as a wall-hit", status === "403" && filed,
    `status=${status} wallHitFiled=${filed}`);
}
...
// ── 4. THE test: raw sockets, no DNS, no proxy, hard-coded addresses ──────────
{
  ...
  let attempts: Record<string, { escaped: boolean; detail: string }> = {};
  try { attempts = JSON.parse(r.stdout); } catch { /* reported below */ }
  const escaped = Object.entries(attempts).filter(([, v]) => v.escaped);
  check("Raw-socket escape attempts all fail",
  ...
  check(v6ReachableUnsealed ? "IPv6 egress is namespaced identically to IPv4"
                            : "IPv6 seal INCONCLUSIVE (no IPv6 egress on this host)",
...
{
  const fd = await docker(["exec", FRONT_DOOR, "sh", "-c",
    "node -e \"require('node:net').connect(443,'1.1.1.1').on('connect',()=>{console.log('reachable');process.exit(0)}).on('error',e=>{console.log('blocked');process.exit(0)})\""], { allowFail: true });
  check("Front door retains egress (record mode is possible)", fd.stdout.includes("reachable"),
...
{
  const egressIp = (await docker(["inspect", "-f",
    `{{(index .NetworkSettings.Networks "${EGRESS_NET}").IPAddress}}`, FRONT_DOOR])).stdout;
...
try:
  ...
  check("App cannot route via the front door's egress interface", r.stdout.includes("blocked"),
